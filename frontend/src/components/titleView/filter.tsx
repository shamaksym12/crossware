...
import { setMediaFilter, setOrder, getReports } from "redux/app/actions";
...
const Filter = () => {
  ...
  const handelPeriod = (date: any[]) => {
    if (date.length) {
      ...
      setEndPeriod(date[1]);
      dispatch(getReports(token, project_id, { from: date[0], to: date[1] }));
    }
  };
  ...
  return (
    <Container className="searchWrap">
      <div className="leftSearch">
        ...
        <Select defaultValue={photographer} className="searchBox member" onChange={handlePhotographer} placeholder={t("report.reporter")}>
        ...
        {visibleCalendar && <div className="calendarWrap">
          <img src={Close} className="closeCalendar" onClick={hideCalendar} />
